...
CI gets its own token. `uv run kinby hub .kinby/hub update-token rotate` prints an update token and refuses the one before it. That token can call `instance.update` and `operation.get` and nothing else. `kinby hub update --connect wss://<hub>/ws <hub-instance-id> --revision <sha>` reads it from `KINBY_TOKEN`, starts the update, prints each step as the hub reaches it, and exits non-zero unless the update succeeds. Add `--package <id> --package-commit <sha>` to move a package installed from git to another commit. kinby's own workflow runs the checks on every pull request, and after a green push to `main` it updates the coder to that commit. It needs the repository secrets `KINBY_HUB_URL` (the contract URL), `KINBY_HUB_UPDATE_TOKEN` and `KINBY_CODER_INSTANCE_ID`, and it skips the update with a notice while `KINBY_HUB_URL` is unset. The hub builds from its own source checkout and does not fetch, so the pushed commit has to be there before the update runs.
