...
`instance.remove` drains an instance and removes its container, and nothing else. The directory, named volumes, secrets and selected image stay, and so does the hub's record. `instance.list` stops showing it, while `instance.list` with `removed` set shows the retained records with their storage. That storage stays reserved, so no other instance can claim it. `instance.stop` with `force` escalates a removal's drain the way it escalates a stop's. `instance.restore` checks that the directory, the named volumes and the selected image are all still there, then creates the container again under the same hub instance id and leaves it stopped for `instance.start`. When something is missing, the restoration fails and names it. It never puts an empty volume or another image in its place.

`instance.delete.preview` lists what deleting a removed instance would take: the resolved directories and named volumes its record owns as writable. Read-only mounts and directories the manifest references are not on the list. `instance.delete` takes those same lists back and deletes exactly them. It refuses when a target changed since the preview, when another instance's record shares the storage, or when a container still exists under the instance's name. Each target is a step of the operation. What failed stays in the record, so a new preview shows what is left to retry. Once everything is gone, `instance.list` no longer shows the record, with or without `removed`.

The hub serves its contract at `GET /ws` on the `--listen` address, `0.0.0.0:8080` by default. On its first start it prints an access token and never prints it again, so store it then. A client sends that token as `Authorization: Bearer <token>`. A browser posts it to `POST /auth/login` and gets a session cookie back, and `POST /auth/logout` ends that session. `uv run kinby hub .kinby/hub token rotate` replaces the token and ends open sessions. Pass `--web-app <dir>` and the hub serves the built web app from it.

CI gets its own token. `uv run kinby hub .kinby/hub update-token rotate` prints an update token and refuses the one before it. That token can call `instance.update` and `operation.get` and nothing else. `kinby hub update --connect wss://<hub>/ws <hub-instance-id> --revision <sha>` reads it from `KINBY_TOKEN`, starts the update, prints each step as the hub reaches it, and exits non-zero unless the update succeeds. Add `--package <id> --package-commit <sha>` to move a package installed from git to another commit. Add `--image-recipe <file>` as well when that commit's image recipe changed, or the hub keeps building with the recipe it recorded. kinby's own workflow runs the checks on every pull request, and after a green push to `main` it updates the coder to that commit. It needs the repository secrets `KINBY_HUB_URL` (the contract URL), `KINBY_HUB_UPDATE_TOKEN` and `KINBY_CODER_INSTANCE_ID`, and it skips the update with a notice while `KINBY_HUB_URL` is unset. The hub builds from its own source checkout and does not fetch, so the pushed commit has to be there before the update runs.
