...
from kinby.memory import GraphStore, MemoryNode, NodeId, RecapWriter
...
def test_failed_recap_appends_warning_without_marker(tmp_path: Path) -> None:
    async def scenario() -> None:
        state_dir = tmp_path / ".state"
        event_log = EventLog(state_dir)
        recap = RecapWriter(event_log, FailingGraphStore(tmp_path))
        dispatcher = build_dispatcher(
            state_dir,
            event_log=event_log,
            turns=TurnConfig(
...
def test_next_turn_starts_while_previous_recap_is_blocked(tmp_path: Path) -> None:
    async def scenario() -> None:
        ...
        recap = RecapWriter(event_log, memory)
        dispatcher = build_dispatcher(
            state_dir,
            event_log=event_log,
            turns=TurnConfig(
