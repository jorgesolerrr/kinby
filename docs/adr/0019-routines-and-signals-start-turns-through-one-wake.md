...
- The scheduler is a reactor in the process that owns the event log (ADR 0006), started with the recap catch-up. `kinby serve` runs that boot without the REPL. A routine turn runs exclusively against all other turns. Two user turns retain the existing per-thread concurrency rule. There is no preemption in v1.
